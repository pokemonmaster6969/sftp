...
import { ArrowLeft, X, Pause, Play, CheckCircle2, FileText, Search, Download as DownloadIcon, Activity, AlertCircle, Clock, Zap, Trash2, List } from 'lucide-react'
import { motion, AnimatePresence } from 'framer-motion'
import type { TransferTask } from './TransferManager'
import { formatBytesOrDash, formatEtaSeconds, formatSpeed } from '../utils'
import { Button, Card, Text, Badge, Group, Stack, Progress, ActionIcon, Title, Paper, ThemeIcon, ScrollArea, Tabs, SimpleGrid, TextInput } from '@mantine/core'
...
export const DownloadsPage: React.FC<DownloadsPageProps> = ({
    ...
    return (
        <div className="min-h-screen bg-slate-50/50 p-4 sm:p-6 lg:p-8 font-sans overflow-y-auto custom-scrollbar">
            <div className="mx-auto">
                {/* Header */}
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
                    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
                        <Group gap={12} mb={10}>
                            <ActionIcon variant="subtle" color="slate" radius="xl" onClick={onClose} size="lg">
                                <ArrowLeft size={20} />
                            </ActionIcon>
                            <Badge variant="filled" color="blue" radius="sm">TRANSFER CONTROL</Badge>
                        </Group>
                        <Title order={1} style={{ fontSize: 42, fontWeight: 900, letterSpacing: -1.5 }}>
                            Downloads <Text component="span" variant="gradient" gradient={{ from: 'blue.6', to: 'indigo.5' }} inherit>Center</Text>
                        </Title>
                        <Text size="lg" c="slate.500" fw={500} className="mt-2">
                            Real-time monitoring of your genomic data streams.
                        </Text>
                    </motion.div>
                ...
                <SimpleGrid cols={{ base: 1, sm: 2, md: 4 }} spacing={24} mb={48}>
                    {[
                        { icon: Activity, label: 'Active Streams', value: stats.active.length, color: 'blue' },
                        { icon: Zap, label: 'Average Speed', value: formatSpeed(stats.avgSpeed), color: 'amber' },
                        { icon: CheckCircle2, label: 'Successful', value: stats.completed.length, color: 'emerald' },
                        { icon: AlertCircle, label: 'Failed Ops', value: stats.errors.length, color: 'red' }
                    ].map((stat, i) => (
                        <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 * i }}>
                            <Paper withBorder radius="20px" p={24} className="bg-white">
                ...
                <div className={`grid grid-cols-1 gap-8 items-start ${selectedTask ? 'lg:grid-cols-12' : ''}`}>
                    {/* Main List Area */}
                    <div className="lg:col-span-8 flex flex-col gap-6">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            ...
                            <TextInput
                                ...
                                size="md"
                                radius="xl"
                                leftSection={<Search size={18} className="text-slate-300 ml-2" />}
                                value={searchQuery}
                        ...
                        <Card padding={0} radius="24px" withBorder bg="white">
                                            ...
                                            filteredTasks.map((task) => {
                                                ...
                                                return (
                                                    <motion.div
                                                        ...
                                                        animate={{ opacity: 1 }}
                                                        exit={{ opacity: 0 }}
                                                        className={`p-5 transition-all cursor-pointer group hover:bg-slate-50/50 ${selected ? 'bg-blue-50/30' : ''}`}
                                                    ...
                                                    >
                                                        <div className="flex items-center gap-5">
                                                            ...
                                                            <div className="flex-1 min-w-0">
                                                                <Text fw={800} size="sm" className="text-slate-900 group-hover:text-blue-600 truncate transition-colors">
                                                                    {task.name}
                                                                </Text>
                                                                <Group gap={12} mt={4}>
                                                            ...
                                                            <div className="flex gap-1">
                                                                {task.status === 'downloading' && (
                                                                    <ActionIcon variant="subtle" color="slate" radius="xl" size="lg" onClick={(e) => { e.stopPropagation(); onPause(task.id); }}>
                                                                        <Pause size={18} />
                                                                    </ActionIcon>
                                                                )}
                                                                {task.status === 'paused' && (
                                                                    <ActionIcon variant="subtle" color="blue" radius="xl" size="lg" onClick={(e) => { e.stopPropagation(); onResume(task.id); }}>
                                                                        <Play size={18} />
                                                                    </ActionIcon>
                                                                )}
                                                                <ActionIcon variant="subtle" color="red" radius="xl" size="lg" onClick={(e) => { e.stopPropagation(); onCancel(task.id); }}>
                                                                    <Trash2 size={18} />
                                                                </ActionIcon>
                                                            </div>
                                                        </div>
                                    ...
                                    <Paper withBorder radius="24px" p={28} className="bg-white shadow-xl shadow-slate-200/50">
                                        <Group justify="space-between" mb={24}>
                                            <div>
                                                ...
                                                <Title order={4} className="leading-tight text-slate-900 group-hover:text-blue-600 line-clamp-2">{selectedTask.name}</Title>
                                            </div>
                                            <ActionIcon variant="subtle" color="slate" radius="xl" onClick={() => setSelectedId(null)}>
                                                <X size={20} />
                                            </ActionIcon>
                                        </Group>
